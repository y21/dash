...
/// Resolves branches from bytecode IPs to emitted cpu offsets.
///
...
impl JumpResolver {
    ...
    pub fn add_user_reference(&mut self, target_bc_ip: Ip, patch_site: PatchSite) -> Option<CpuIp> {
    ...
    pub fn resolve_user_label(&mut self, bc_ip: Ip, x86_ip: CpuIp) -> Vec<PatchSite> {
    ...
    pub fn add_internal_reference(&mut self, label: InternalLabel, patch_site: PatchSite) -> Option<CpuIp> {
    ...
    pub fn resolve_internal_label(&mut self, label: InternalLabel, x86_ip: CpuIp) -> Vec<PatchSite> {
        let slot = self.internal_labels.entry(label).or_default();
